...
V_L_te(k,l) = V_L_te(i-1,2);
I_L_te(k,l) = I_L_te(i-1,2);
    end
end
[R,C]= find(P_tot<=50 & P_tot>= 20);
for j = 1:length(R);
P_new_tot(j)= P_tot(R(j),C(j));
T_pv_new(j) = T_pv(R(j),C(j));
Eta_new_tot(j) = Eta_tot(R(j),C(j));
end
T_P_tot = vertcat(P_new_tot,T_pv_new, n_k(C),rc_i(R),Eta_new_tot);
[X,Y] = meshgrid(rc_i,n_k);
subplot(2,2,1)
plot(rc_i,P_tot);
title('Subplot 1: Total Power vs Variations in Concentration Ratio');
xlabel('Concentration Ratios')
ylabel('Total Power (W)')
legendCell = cellstr(num2str(n_k', 'n=%-d'));
...
legendCell = cellstr(num2str(n_k', 'n=%-d'));
legend(legendCell, 'Location', 'southeast')
title('Subplot 2: Total Efficiency vs Variations in Concentration Ratio');
subplot(2,2,[3,4])
plot(rc_i,T_pv);
xlabel('Concentration Ratios')
ylabel('Temperature of the PV Cell (K)')
legendCell = cellstr(num2str(n_k', 'n=%-d'));
legend(legendCell, 'Location', 'southeast')
title('Subplot 3: T_PV vs Variations in Concentration Ratio');
surf(X,Y,Eta_tot)
rotate3d on
surf(X,Y,Eta_tot)
rotate3d on
xlabel('Concentration Ratios')
ylabel('Thermoelectric Pairs')
zlabel('Total Efficiency')
title('Total Efficiency vs Concentration ratio vs No. of Thermoelectric Pairs')
surf(X,Y,P_tot)
rotate3d on
xlabel('Concentration Ratios')
ylabel('Thermoelectric Pairs')
zlabel('Total Power')
title('Total Power vs Concentration ratio vs No. of Thermoelectric Pairs')
surf(X,Y,T_pv)
rotate3d on
xlabel('Concentration Ratios')
ylabel('Thermoelectric Pairs')
zlabel('Temperature of Photovoltaic Cell (K)')
title('T_pv vs Concentration ratio vs No. of Thermoelectric Pairs')
