function [Power, Heat, T5_f, alpha, W_c, W_t, n_comp,T3, T2_r,cp_23]= task_5(~)
Qs= [0,500,1000,1500,2000,2500,3000];
gamma= 0.25;
P1= 101;
T1=298;
P2=500;
...
for l = 1:length(Qs)
    i = 1;
    T5_f(1,l) =1000;
alpha(1,l) = alpha_stoich;
while i<4
    m_prod(i,l) = 28.014*3.76*alpha(i,l)/4.76 + 15.9999*2 *(alpha(i,l)/4.76 - 3*gamma -2)+18.01528*(2+2*gamma)+44.01*(1+2*gamma);
    m_air = 6;
    n_dot_air(i,l)= m_air/28.97;
    n_prod(i,l) = 3.76*alpha(i,l)/4.76 + (alpha(i,l)/4.76 - 3*gamma -2)+(2+2*gamma)+(1+2*gamma);
    ...
    [y_h20(i,l), y_co2(i,l), y_n2(i,l), y_o2(i,l), cp_prod(i,l), cp_air(i,l)] = task_3(T_avg_25, gamma,alpha(i,l));
    ...
    T2_r(i,l) = T2_final(i,l)+ (ncp_min* eps_regen* (T5_f(i,l)- T2_final(i,l)))/(n_dot_air(i,l)*cp_air(i,l));
     %disp(i);
     %disp(n_dot_air(i,l));
    %[T3(i,l), cp_23(i,l)] = root_finder(Qs(l),T2_r(i,l),n_dot_air(i,l));
    cp_23(i,l) = 28.11+ 0.1967*10^(-2)*((T2_r(i,l))) + 0.4802*10^(-5)*((T2_r(i,l)))^2 - 1.966*10^(-9)*((T2_r(i,l)))^3;
    T3(i,l) = Qs(l)/(n_dot_air(i,l)*cp_23(i,l))+T2_r(i,l);
    i = i+1;
    alpha(i,l)= task_1(T4, T3(i-1,l), gamma);
end
...
[~, ~, ~, ~, cp_burn, ~] = task_3(.5*(T3(i-1,l)+T4), gamma,alpha(3,l));
Heat(1,l) =(n_dot_prod(3,l)*cp_burn*(T4-T3(i-1,l)));
n_comp(1,l) = (n_dot_prod(3,l)*cp_5(3,l)*(T4-T5_f(3,l))- n_dot_air(3,l)*cp_2(3,l)*(T2_final(3,l)-T1))/(Heat(1,l)+Qs(l));
end
subplot(2,2,1)
plot(Qs, Power )
xlabel('Q_{solar} (KW)')
...
plot(Qs, Heat)
xlabel('Q_{solar} (KW)')
ylabel('Heat Input in combustor (KW)')
subplot(2,2,2)
plot(Qs, n_comp)
xlabel('Q_{solar} (KW)')
